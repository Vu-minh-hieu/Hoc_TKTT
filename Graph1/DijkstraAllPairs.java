/******************************************************************************
 *  Compilation:  javac DijkstraAllPairs.java
 *  Execution:    java DijkstraAllPairs input.txt s
 ...
 *
 *  % java DijkstraAllPairs "tinyEWD.txt", "0"
 *  0 to 0 (0.00)  
 ...
 *
 *  % java DijkstraAllPairs mediumEWD.txt 0
 *  0 to 0 (0.00)  
...
public class DijkstraAllPairs {
    ...
    private void relax(DirectedEdge e, int i) {
        int v = e.from(), w = e.to();
        if (distTo[i][w] > distTo[i][v] + e.weight()) {
            distTo[i][w] = distTo[i][v] + e.weight();
            edgeTo[i][w] = e;
            if (pq.contains(w)) pq.decreaseKey(w, distTo[i][w]);
            else                pq.insert(w, distTo[i][w]);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        DijkstraAllPairs sp = new DijkstraAllPairs(G);
        
        // print shortest path
        for(int s = 0; s < G.V(); s++){
            for (int t = 0; t < G.V(); t++) {
                if (sp.hasPathTo(s,t)) {
                    StdOut.printf("%d to %d (%.2f)  ", s, t, sp.distTo(s,t));
                    for (DirectedEdge e : sp.pathTo(s,t)) {
                        StdOut.print(e + "   ");
                    }
                    StdOut.println();
                }
